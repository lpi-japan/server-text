...
### example.comというドメイン名
example.comは、RFC 2606で例示用に定義されているドメイン名です。例示用ですが問題なく動作しますので、サーバーの設定に慣れないうちはこのまま設定してください。その他のホスト名、ドメイン名も設定できますが、以後の説明で使用されているホスト名やドメイン名はすべて読み替えるようにしてください。
