...
インターネット接続できない環境での実習の場合、OSインストール時にメールサーバーを追加でインストールしていますが、s-nailパッケージはインストールされていません。同様にThunderbird（thunderbirdパッケージ）もインストールされていません。
...
入力を終えたら「完了」ボタンをクリックします。「警告！」ダイアログが表示されますが、左下の「接続する上での危険性を理解しました」をチェックし、「確認」ボタンをクリックします。
