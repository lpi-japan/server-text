...
\newif\ifbibliography
$endif$
% Define passthrough command for Pandoc compatibility (required for all versions)
...
% Compatibility with recent pandoc longtable output that may set \LTcaptype{none}
\makeatletter
\@ifundefined{c@none}{\newcounter{none}}{}
\makeatother
$if(beamer)$
...
% Macro for scaling images that don't have explicit size options
\makeatletter
...
\begin{center}
  \vspace*{6cm}
  {\Huge\linespread{1.2}\selectfont $bookTitle$\par}

  \vfill

  {\Large $bookVersion$}

  {\Large LPI-Japan \, $if(publisher-suffix)$$publisher-suffix$$else$発行$endif$}
...
%% \includepdf[pages=-, noautoscale=true, scale=1.0]{PDF用表紙.pdf}
%% \thispagestyle{empty}
%% \includepdf[pages=-, noautoscale=true, scale=1.0]{中表紙前.pdf}
%% \thispagestyle{empty}

\pagenumbering{roman}
%% \markdownInput{Chapter0.md}
$for(include-before)$
$include-before$
...
\thispagestyle{empty}
  \vspace*{\fill}
  {\LARGE\linespread{1.2}\selectfont $bookTitle$\par}

  \rule{\textwidth}{0.5pt}

  $bookVersion$

  $if(publisher-prefix)$$publisher-prefix$$else$発行元$endif$ \, LPI-Japan

  \rule{\textwidth}{0.5pt}

  Copyright \copyright{} LPI-Japan. All Rights Reserved.
