...
## ネットワークの追加 
作成した仮想マシンを選択した状態で設定(S)をクリックします。
ネットワークタブから追加したいネットワークの数だけアダプタの追加を行います。
デフォルトではアダプタ1のみ有効となっています。
...
# ネットワーク接続の確認
ネットワークが設定されているのかip a　コマンドを実行して確認を行います。　

## インターネット接続
起動していない場合はnmtui コマンドを実行し、ネットワークを起動させます。 
...
## unbound-keygenの起動
設定が終わったら、unboundの制御に使う鍵の生成を行います。次のように、unbound-keygenを起動すると、鍵が自動的に生成されます。 

systemctl start unbound-keygen

## unboundの設定確認
設定を行ったら、書式のチェックを行っておきましょう。
「no errors」と表示されていれば、書式エラーがないということです。エラーがある場合には、エラーの内容が表示されます。

エラーが出た場合には、内容を確認して修正します。
...
# サービスの起動/停止と動作確認
設定が終わったら、unboundを起動しましょう。systemctlで、unboundユニットを起動します.
## サービスの起動
...
Activeの欄に「active (running)」と表示されていることを確認します。また、下の方にはログが表示されます。ここでも、「info: startof service」と表示されていて、unboundのサービスが起動していることが確認できます。
