...
![マウントとブートのリトライダイアログ画面](image/Ch3/BootDialog.png)
...
![言語選択画面](image/Ch3/selectLang.png)
...
![インストール概要画面](image/Ch3/installOverView.png)
...
![インストール先画面](image/Ch3/installDisk.png)
...
![NATネットワークのインターフェース画面](image/Ch3/installNetNAT.png)
...
![ホストオンリーネットワークのインターフェース画面](image/Ch3/installNetHostOnly.png)

1. 「enp0s8の編集」ダイアログが表示されるので、「IPv4 設定」タブを選択します。
1. 「メソッド(M)」を「手動」に変更します。
1. 「アドレス」で「追加」をクリックし、各項目を以下の値に設定します。
...
![ホストオンリーネットワークのインターフェース設定画面](image/Ch3/installNetHostOnlyConfig.png)
...
![ホスト名の設定画面](image/Ch3/installNetHostName.png)
...
![ソフトウェアの選択画面](image/Ch3/selectSoftware.png)
...
![ソフトウェアの追加選択画面](image/Ch3/selectSoftwareCustom.png)
...
![rootパスワードの設定画面](image/Ch3/setRootPassword.png)

### インストールの開始
必要な設定が終わったら、インストールを開始します。「インストール開始」ボタンをクリックします。

![インストール開始前の画面](image/Ch3/startInstall.png)

インストールが始まり、「インストール状況の表示」画面が表示されます。

インストールが終了したら、「システムの再起動」ボタンをクリックして、再起動します。

![インストール終了の画面](image/Ch3/finishInstall.png)
...
![セットアップ開始の画面](image/Ch3/startSetup.png)
...
![プライバシー設定の画面](image/Ch3/Privacy.png)
...
![オンラインアカウント設定の画面](image/Ch3/OnlineAccount.png)
...
![ユーザー名設定の画面](image/Ch3/UserName.png)

ユーザーadminのパスワードも設定します。rootパスワード同様、「パスワード」と「確認」に同じパスワードを入力します。

設定したら、「次へ(N)」ボタンをクリックします。

![パスワード設定の画面](image/Ch3/Password.png)

すべての設定を完了したら、「AlmaLinuxを使い始める(S)」ボタンをクリックします。

![AlmaLinuxを使い始めるの画面](image/Ch3/startAlmaLinux.png)
...
![ログアウト選択の画面](image/Ch3/Logout.png)

### ログインする方法
ログインするには、ログインしたいユーザーをクリックして選択し、パスワードを入力します。

![ログインの画面](image/Ch3/Login.png)
...
![端末起動の画面](image/Ch3/Terminal.png)

また、仮想マシンで直接操作するのではなく、ホストOSからSSHを使ってリモート接続して操作することもできます。方法については第7章で解説します。
...
1. 画面左下の「ここに入力して検索」に「cmd」と入力します。
1. 「コマンドプロンプト」が検索されるので、クリックして起動します。
1. pingコマンドを実行します。
